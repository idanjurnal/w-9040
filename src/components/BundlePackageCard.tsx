...
const BundlePackageCard = ({
  ...
  return <div className="group transition-all duration-300 hover:scale-[1.02] p-4">
      <div className="text-center mb-6">
          <h4 className="text-4xl lg:text-5xl font-serif text-stone-700 tracking-widest uppercase">{bundle.title}</h4>
      </div>
      
      {bundle.subPackages ? <div className="grid md:grid-cols-5 gap-8 lg:gap-12 items-start">
          ...
          <div className="md:col-span-3 pt-4">
              {bundle.subPackages.map((subPkg, subIndex) => <div key={subIndex} className="mb-8">
                  <h5 className="text-xl font-serif text-stone-700 border-b border-stone-300 pb-2 mb-4">{subPkg.title}</h5>
                  <ul className="space-y-2">
                    {subPkg.features.map((feature, featureIndex) => <li key={featureIndex} className="text-sm text-stone-600 font-light flex">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
              {bundle.images && bundle.images.map((img, imgIndex) => <img key={imgIndex} src={img} alt={`${bundle.name} collage ${imgIndex + 1}`} className="rounded-lg shadow-lg w-full h-auto object-cover aspect-square" />)}
          </div>
          <ul className="columns-1 sm:columns-2 md:columns-3 gap-x-8 space-y-2 text-center">
              {bundle.features && bundle.features.map((feature, featureIndex) => <li key={featureIndex} className="text-sm text-stone-600 font-light break-inside-avoid">
                    {feature}
                </li>)}
          </ul>
        </>}
